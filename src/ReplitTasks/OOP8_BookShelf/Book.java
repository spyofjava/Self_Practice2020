...
public class Book {
     ...
     */
    public static String capitalize(String word) { // it accepts single word
     ...
     */
    public static Boolean isTitleWord(String word) {//it also accepts a single word==> Acaba Title olabilir mi? onu check edecek.

        ArrayList<String> listofArticle = new ArrayList<>(Arrays.asList("at", "by", "in", "into", "near", "of", "on", "to", "than", "via", "and", "but", "for", "nor", "or", "so", "yet", "a", "an", "the"));

        if (listofArticle.contains(word)) {
            return false;
        } else {
            return true;
        }
    ...
    public void setAuthor(String author) {
        if (author.isEmpty()) {
            ...
            String[] authorFullname = author.split(" ");//  [ali,koyuncu]
            String result = "";
            for (int i = 0; i < authorFullname.length; i++) {
                if (i != authorFullname.length - 1) {
                    result += capitalize(authorFullname[i]) + " ";
                } else {
                    result += capitalize(authorFullname[i]);
                }
            }
    ...
    public void setTitle(String title) {
        if (!title.contains(" ")) {
            ...
            this.title = "";
        } else if (title.contains(" ")) {//baslik cok kelimeli
            title=title.toLowerCase();
            String[] titlelist = title.split(" ");// [jack  box]

            String result = "";
            for (int i = 0; i < titlelist.length; i++) {

                if (i == 0) {
                    result += capitalize(titlelist[i])+" ";
                } else {
                    if (isTitleWord(titlelist[i])) {//ve article degilse capitalize edecek
                    ...
                    } else {// if it is article, it'll write without making uppercase --->article ise buyukharf yapmadan yazacak
                        if (i != titlelist.length - 1) {
                            result += (titlelist[i].toLowerCase()) + " ";
                        } else {
                            result += (titlelist[i].toLowerCase()+"");
                        }
                    }
